...
export function CreateTaskForm() {
  ...
  const onSubmit = async (values: z.infer<typeof taskFormSchema>) => {
    ...
    const { title, deadline, description, isCompleted } = values
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: data?.message || "Ocorreu um erro ao adicionar a tarefa"
        })
      }
    } catch (error) {
      toast({
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          ...
          <FormField
            ...
            name="isCompleted"
            render={({ field }) => (
              <FormItem className="col-span-1 flex flex-row items-center justify-start space-x-3 ">
                <FormControl>
                  <Checkbox
                    checked={field.value}
